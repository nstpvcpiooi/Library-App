...
public class MysqlReviewDAOImpl implements ReviewDAO {
    private static MysqlReviewDAOImpl instance;

    private MysqlReviewDAOImpl() {
    }

    public static MysqlReviewDAOImpl getInstance() {
        if (instance == null) {
            synchronized (MysqlReviewDAOImpl.class) {
                if (instance == null) {
                    instance = new MysqlReviewDAOImpl();
                }
            }
        }
        return instance;
    ...
    public int getRatingCountForBook(String bookID) {
        String query = "SELECT COUNT(*) AS ratingCount FROM Reviews WHERE bookID = ?";
        try (Connection connection = JDBCUtil.getConnection();
             PreparedStatement statement = connection.prepareStatement(query)) {
            statement.setString(1, bookID);
            ResultSet resultSet = statement.executeQuery();
            if (resultSet.next()) {
                return resultSet.getInt("ratingCount");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
